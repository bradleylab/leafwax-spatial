...
spatial_cv_analysis <- function(model_name, prepared_data_dir = NULL) {
  ...
  if (stan_data$include_gp == 1) {
    # For each observation, find the data density of its nearest knot.
    # Observations and knots are both in 3-D chordal coordinates (km), so
    # Euclidean distance over those coordinates is chordal distance in km.
    obs_to_knot <- fields::rdist(stan_data$coords, stan_data$knot_coords)
    nearest_knot <- apply(obs_to_knot, 1, which.min)
    coords$nearest_knot_density <- stan_data$knot_data_density[nearest_knot]
...
write.csv(s2_csv,
          file.path(.tables_out, "Table_S2_regional_performance.csv"),
          row.names = FALSE)

# Wide RMSE-only body used by the compact Communications Earth & Environment
# Supplementary Table S3. It is generated from the same rows as the detailed
# CSV so the manuscript never carries a hand-copied second version.
s3_regions <- c("Overall", "Africa", "Americas", "Asia", "Europe", "Oceania")
s3_wide <- s2_csv |>
  select(model, region, rmse) |>
  mutate(region = factor(region, levels = s3_regions)) |>
  tidyr::pivot_wider(names_from = region, values_from = rmse) |>
  arrange(match(model, s2_model_order))

s3_lines <- vapply(seq_len(nrow(s3_wide)), function(i) {
  r <- s3_wide[i, ]
  sprintf(
    "%s & %.1f & %.1f & %.1f & %.1f & %.1f & %.1f \\\\",
    .latex_model_name_s2(r$model), r$Overall, r$Africa, r$Americas,
    r$Asia, r$Europe, r$Oceania
  )
}, character(1))

s3_path <- file.path(.tables_out, "Table_S3_regional_rmse_body.tex")
writeLines(c(
  "% Auto-generated by 5b_overfitting_diagnostics.R.",
  "% Do not hand-edit — regenerate from the frozen model outputs.",
  "",
  s3_lines
), s3_path)
cat("wrote", s3_path, "\n")

s2_path <- file.path(.tables_out, "Table_S2_regional_performance_body.tex")
writeLines(c(
  "% Auto-generated by 5b_overfitting_diagnostics.R.",
