#!/bin/bash
#SBATCH --job-name=leafwax_6a
#SBATCH -A compute2-alexander.s.bradley
#SBATCH --partition=general-cpu
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH --time=24:00:00
#SBATCH --array=0-2
#SBATCH --output=logs/6a_%A_%a.out
#SBATCH --error=logs/6a_%A_%a.err

set -eo pipefail

WORKDIR=/scratch2/fs1/alexander.s.bradley/leafwax_run
SIF=${WORKDIR}/leafwax-spatial.sif

module load ris
module load apptainer/1.3.6

cd "${WORKDIR}"
mkdir -p logs

SCENARIOS=(3a 3b 3c)
SCEN=${SCENARIOS[$SLURM_ARRAY_TASK_ID]}

echo "=== 6a simulated recovery: scenario ${SCEN} ==="
echo "Array task: ${SLURM_ARRAY_TASK_ID}"
echo "Node: $(hostname)"
echo "Start: $(date)"

apptainer exec --no-home --containall \
    --bind "${WORKDIR}:${WORKDIR}" \
    --bind /tmp:/tmp \
    --pwd "${WORKDIR}" \
    --env CMDSTAN=/root/.cmdstan/cmdstan-2.36.0 \
    --env OPENBLAS_CORETYPE=Haswell \
    "${SIF}" \
    Rscript 6a_simulated_recovery.R "${SCEN}"

echo "Done: $(date)"
